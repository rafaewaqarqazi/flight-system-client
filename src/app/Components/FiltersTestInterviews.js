...
const FiltersTestInterviews = ({filters, handleChangeFilters}) => {
    ...
    <div className='d-flex justify-content-sm-end flex-wrap align-items-center '>
      <Dropdown isOpen={dropdown} toggle={() => setDropdown(!dropdown)}>
        <DropdownToggle
          ...
          caret
        >
          {filters.status !== '' ? getTestInterviewStatus(filters.status) : 'Select Status'}
        </DropdownToggle>
        <DropdownMenu>
          ...
          <DropdownItem onClick={() => handleChangeFilters('status', '2')}>Passed</DropdownItem>
          <DropdownItem onClick={() => handleChangeFilters('status', '3')}>Failed</DropdownItem>
        </DropdownMenu>
      </Dropdown>
...
export default FiltersTestInterviews;